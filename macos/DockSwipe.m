...
void TMCDockSwipeEndWithVelocity(double velocityX) {
    ...
    // Double-send after 200ms (Mac Mouse Fix reliability pattern against WindowServer drop under load)
...
void TMCDockSwipeCommit(void) {
    ...
    double startOffset = ApplyRubberBand(sCurrentOffset);
    double exitSpeed = (startOffset < 0.0) ? -3.5 : 3.5;
    PostDockSwipeFrame(startOffset, 1, 4 /* Ended */, exitSpeed);
}
...
void TMCTriggerSpaceSwipe(TMCSpaceDirection direction) {
    ...
    double signedOffset = (direction == TMCSpaceDirectionNext) ? -baseOffset : baseOffset;
    if (!IsNaturalScrollEnabled()) {
        signedOffset = -signedOffset;
    }

    // Direct native trackpad flick: Began -> Ended with velocity
    PostDockSwipeFrame(0.0, 1, 1 /* Began */, 0.0);
    double exitSpeed = (signedOffset < 0.0) ? -3.5 : 3.5;
    PostDockSwipeFrame(signedOffset * 0.15, 1, 4 /* Ended */, exitSpeed);
}
